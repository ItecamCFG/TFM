...
class GeneticMakespanPlanner(OptimizationModel):
    ...
    def _build_model(self):
        """No se construye un modelo formal; el GA genera soluciones directamente."""
        pass
    ...
    def _solve_model(self, time_limit: Optional[int] = None) -> tuple[str, float]:
        ...
        # Horizon length for feasibility checks
        self.M_days = len(self.days_list)
        # Mapping resource -> expertise for quick lookup
        self.resource_expertise_map = self.expertise_dict

        # Use the full list of task objects directly
        all_tasks_flat: List[Task] = list(self.input_data.tasks)
    ...
    def _evaluate_and_decode(self, individual_permutation: List[str]) -> Tuple[List[str], float, Dict, Dict]:
        ...
        for tid, end_time in task_end_times.items():
            if end_time != float('inf'):
                task_completion_days_result[(self.task_dict[tid].project_id, tid)] = int(math.ceil(end_time))
            else:
                task_completion_days_result[(self.task_dict[tid].project_id, tid)] = None
